function [k_laplace,R_squeare_real_axial,R_squeare_real_later,R_squeare_real_1d, k_axial,k_later,k_1d,R_cc_ax,R_cc_la, R_cc_1d] = test_reverb2(M_filtered,res_x,res_y,mode,correc)
if mode==1
    %tic
...
for angle=1:1:360
    ...
    lapla=real(del2(sim_autocorr,res_x,res_y));
    k_laplace=abs(lapla(round((size(corre,1)+1)/2),round((size(corre,2)+1)/2)));
    k_laplace= sqrt(5*k_laplace/3);
end 