function [hF,hB] = imoverlay2(B,F,climF,climB,cmap,alpha,haxes,xdim,ydim)
...
if isempty(haxes) || ~ishandle(haxes)
    ...
    set(f,'Position',[pos(1),pos(2),size(B,2)+620,size(B,1)+100]);
    haxes = axes;
...
axes(haxes);
ax1 = subplot(1,2,1);
imagesc(xdim,ydim,B);%axis image on;
xlabel('\bf Lateral [cm]')
ylabel('\bf Depth [cm]')
title('B-mode image'); colormap(ax1,gray)
h1 = colorbar; ylabel(h1,'dB'); 
set (gca,'clim',[-40 0]);
set(gca,'fontsize',24,'fontname','times','fontweight','b');

ax2 = subplot(1,2,2);
hB = imagesc(xdim,ydim,B);%axis image on;
xlabel('\bf Lateral [cm]')
ylabel('\bf Depth [cm]')
title('SWS image'); colormap(ax2,cmap)
h2 = colorbar; ylabel(h2,'m/s'); colormap jet;
set(gca,'fontsize',24,'fontname','times','fontweight','b');
